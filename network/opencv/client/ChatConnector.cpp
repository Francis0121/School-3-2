...
void CChatConnector::OnBnClickedOk()
{
	...
	if(!m_client_socket->Create()){
		AfxMessageBox(_T("Message 소켓 생성을 실패했습니다,"));
		return; 
	}

	if(!m_client_socket->Connect(t_ip, port)){
		AfxMessageBox(_T("Message 서버와 연결을 실패하였습니다"));
		m_client_socket->Close();
		return;
	}
	// ~ image socket 하나 더 생성 
	port+=1;

	// ~ 성공한 경우 0이 아닌 값 Return
	if(!m_image_socket->Create()){
		AfxMessageBox(_T("Image 소켓 생성을 실패했습니다,"));
		return; 
	}

	if(!m_image_socket->Connect(t_ip, port)){
		AfxMessageBox(_T("Image 서버와 연결을 실패하였습니다"));
	...
	if(!m_cam_socket->Create()){
		AfxMessageBox(_T("Cam 소켓 생성을 실패했습니다,"));
		return; 
	}

	if(!m_cam_socket->Connect(t_ip, port)){
		AfxMessageBox(_T("Cam 서버와 연결을 실패하였습니다"));
		m_cam_socket->Close();
		m_image_socket->Close();
		m_client_socket->Close();
		return;
