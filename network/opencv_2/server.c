...
int list_client_cam[MAX_CLIENT];
char greeting[]="Welcome to chatting room\n";
char CODE200[]="Sorry No More Connect";

main(int argc, char *argv[])
{
	int s_socket, c_socket, c_whiteBoard_socket, c_cam_socket;
	...
	for(i = 0; i < MAX_CLIENT; i++){
		...
		list_client_cam[i] = INVALID_SOCK;
	}
	...
	while(1)
	{
		...
		c_cam_socket = accept(s_socket, (struct sockaddr *)&c_addr, &len);
		printf("c_cams_socket accept sucess\n");
		res = pushClient(c_socket, c_whiteBoard_socket, c_cam_socket);
		if(res < 0)
		{
		...
		else
		{
			...
			pthread_create(&thread, NULL, do_cam, (void *)c_cam_socket);
		}
	}
}
...
void *do_cam(void *arg)
{
	...
	while(1){
		...
		if((n = read(c_socket, chatData, sizeof(chatData))) > 0){
			for(i = 0; i < MAX_CLIENT; i++)
				if(list_client_cam[i] != INVALID_SOCK && list_client_cam[i] != c_socket)
					write(list_client_cam[i], chatData, n);
		}
		else
		{
				popClient(c_socket);
...
int pushClient(int c_socket, int c_whiteBoard_socket, int c_cam_socket)
{
	int i;
	...
	for(i = 0; i < MAX_CLIENT; i++)
	{
		...
		if(list_c[i] == INVALID_SOCK)
		{
			...
			list_client_cam[i] = c_cam_socket;
